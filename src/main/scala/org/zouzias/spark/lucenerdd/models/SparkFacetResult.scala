...
object SparkFacetResult extends Serializable {
  ...
  def apply(facetResult: FacetResult): SparkFacetResult = {
    val facets = facetResult.labelValues.map( labelValue =>
      labelValue.label -> labelValue.value.longValue()
    ).toMap[String, Long]

    SparkFacetResult(facetResult.dim, facets)
  }
}
