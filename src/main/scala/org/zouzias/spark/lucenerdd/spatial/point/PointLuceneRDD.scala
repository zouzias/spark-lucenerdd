...
object PointLuceneRDD {
   ...
   * @return
   */
  def apply[K <% (Double, Double) : ClassTag, V: ClassTag](elems: RDD[(K, V)])
  : PointLuceneRDD[K, V] = {
   ...
   * @return
   */
  def apply[K <% (Double, Double) : ClassTag, V: ClassTag]
  (elems: Iterable[(K, V)])(implicit sc: SparkContext): PointLuceneRDD[K, V] = {
